...
def generate(config: str = typer.Option(..., "--config"), count: int = typer.Option(48, "--count")):
    ...
    rows = run_generate(cfg, count)
    typer.echo(json.dumps({"generated": len(rows), "data_path": cfg.data_path}))


@app.command()
def annotate(config: str = typer.Option(..., "--config")):
    cfg = load_config(config)
    signals, labels = run_annotate(cfg)
    typer.echo(
        json.dumps(
            {
                "signals": len(signals),
                "aggregated": len(labels),
                "accepted": sum(not row.abstained for row in labels),
                "run_dir": cfg.run_dir,
            }
        )
    )
...
def train(config: str = typer.Option(..., "--config")):
    ...
    typer.echo(json.dumps({"checkpoint": str(checkpoint)}))


@app.command()
def evaluate(config: str = typer.Option(..., "--config"), checkpoint: str = typer.Option(..., "--checkpoint")):
    report = run_evaluation(load_config(config), checkpoint)
    typer.echo(json.dumps({key: value for key, value in report.items() if key != "student_predictions"}, indent=2))
...
def predict(
    ...
    result = run_predict(module, [Sample(id="prediction", text=text, image=image)])[0]
