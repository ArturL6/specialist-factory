...
def _signal(
    ...
    signal_type: SignalType = SignalType.MODEL_PROBABILITY,
    **metadata: Any,
) -> TeacherSignal:
    ...
    probabilities = (
        ...
        if total
        else {key: 1 / len(probabilities) for key in probabilities}
    )
    label = max(probabilities, key=probabilities.get)
    return TeacherSignal(
        sample_id=sample.id,
        teacher_id=cfg.id,
        teacher_model=cfg.model_name or cfg.model or cfg.type,
...
class MockTeacher:
    ...
    def predict(self, samples: list[Sample], task: TaskDefinition) -> list[TeacherSignal]:
        ...
        for sample in samples:
            ...
            if "cancel" in haystack and "cancel_order" in scores:
                scores["cancel_order"] = 0.75
            elif any(word in haystack for word in ("refund", "money back", "return")) and "refund" in scores:
                scores["refund"] = 0.75
            elif any(word in haystack for word in ("where", "track", "package", "order")) and "order_status" in scores:
                scores["order_status"] = 0.75
            elif sample.image and {"scratch", "clean"}.issubset(scores):
                from PIL import Image

                red, green = (
                    __import__("numpy")
                    .asarray(Image.open(sample.image).convert("RGB"), dtype=float)
                    .mean(axis=(0, 1))[:2]
                )
                scores["scratch" if red > green else "clean"] = 0.8
            elif "other" in scores:
                scores["other"] = 0.65
            signals.append(
                _signal(
                    sample,
...
class HuggingFaceTextTeacher:
    def __init__(self, cfg: TeacherConfig):
        self.cfg, self.teacher_id, self._pipe = cfg, cfg.id, None
    ...
    def predict(self, samples: list[Sample], task: TaskDefinition) -> list[TeacherSignal]:
        ...
        for sample in samples:
            ...
            result = self._pipeline()(
                sample.text, candidate_labels=labels, multi_label=False, hypothesis_template="This request is about {}."
            )
            probabilities = {
            ...
            output.append(
                _signal(
                    ...
                    source="transformers.zero-shot-classification",
                    model_revision=self.cfg.version,
                    cost=0,
                )
            )
        ...
        for sample in samples:
            ...
            output.append(
                _signal(
                    ...
                    probs,
                    latency_ms=round((time.perf_counter() - started) * 1000, 2),
                    source="transformers.zero-shot-image-classification",
                    model_revision=self.cfg.version,
                    cost=0,
                )
            )
...
class OpenRouterVisionTeacher:
    ...
    def predict(self, samples: list[Sample], task: TaskDefinition) -> list[TeacherSignal]:
        ...
        if not key:
            raise RuntimeError("OpenRouter key missing; set the configured api_key_env")
        url = (self.cfg.base_url or os.getenv("OPENROUTER_BASE_URL", "https://openrouter.ai/api/v1")).rstrip(
            "/"
        ) + "/chat/completions"
        ...
        for sample in samples:
            prompt = (
                f"Classify against {classes}. Return JSON only: "
                f"{{'label': one class name, 'confidence': 0..1}}. Text: {sample.text or ''}"
            )
            ...
            if label not in task.labels:
                raise ValueError(f"VLM returned invalid label {label!r}")
            confidence = float(parsed.get("confidence", 1.0))
            probs = {item: 0.0 for item in task.labels}
            probs[label] = 1.0
            output.append(
                _signal(
                    sample,
                    self.cfg,
                    probs,
                    SignalType.SELF_REPORTED_PROBABILITY,
                    latency_ms=round((time.perf_counter() - started) * 1000, 2),
                    cost=float(raw.get("usage", {}).get("cost", 0) or 0),
                    provider="openrouter",
                    raw_label=label,
...
def build_teacher(cfg: TeacherConfig):
    ...
    if cfg.type not in types:
        raise ValueError(f"Unsupported teacher type: {cfg.type}")
