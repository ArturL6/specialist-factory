...
def _intent_examples() -> list[tuple[str, str]]:
    return [
        ...
        ("Track order number 17", "order_status"),
        ("My delivery has not arrived", "order_status"),
        ("Please cancel my order", "cancel_order"),
        ("I ordered by mistake, cancel it", "cancel_order"),
        ("Stop shipment immediately", "cancel_order"),
        ("I want my money back", "refund"),
        ("Please issue a refund", "refund"),
        ("The item arrived broken; return my payment", "refund"),
        ("What are your opening hours?", "other"),
        ("Can I change my account email?", "other"),
...
def generate(config: AppConfig, count: int = 48) -> list[Sample]:
    ...
    image_dir = data_path.parent / "images"
    ...
    for index in range(count):
        ...
        if modality in ("image", "multimodal"):
            ...
            palette.update(
                {
                    "scratch": ((220, 60, 60), "scratch"),
                    "clean": ((70, 180, 80), "clean"),
                    "damaged": ((220, 60, 60), "damage"),
                    "undamaged": ((70, 180, 80), "clear"),
                }
            )
            ...
            if tag in ("scratch", "damage"):
                draw.line((20, 20, 76, 76), fill="black", width=7)
            draw.text((5, 5), label[:8], fill="black")
            canvas.save(image)
        ...
        metadata = {
            "synthetic": True,
            "generator": "builtin-v1",
            "label_source": "synthetic_gold_proxy" if human else "unlabeled",
        }
