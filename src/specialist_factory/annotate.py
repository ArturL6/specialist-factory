...
def annotate(config: AppConfig) -> tuple[list[TeacherSignal], list[AggregatedLabel]]:
    ...
    cache = TeacherCache(config.cache_path)
        ...
        if misses:
            ...
            for sample in misses:
                if sample.id not in by_id:
                    # Teacher legitimately skipped this sample (e.g. missing modality); retry later.
                    continue
                key = cache.key(sample, config.task, teacher_cfg)
                try:
                    signal = by_id[sample.id]
                    validate_signal(signal, config.task)
                    cache.put(
                        key,
                        signal,
                        latency_ms=signal.metadata.get("latency_ms", 0),
                        cost=signal.metadata.get("cost", 0),
                    )
                    all_signals.append(signal)
                except Exception as exc:
                    cache.put(key, None, error=str(exc))
                    raise
    ...
    for signal in all_signals:
        grouped[signal.sample_id].append(signal)
...
def load_annotations(config: AppConfig) -> tuple[list[TeacherSignal], dict[str, AggregatedLabel]]:
    run = Path(config.run_dir)
    signals = [
        TeacherSignal.model_validate_json(line) for line in (run / "teacher_signals.jsonl").read_text().splitlines()
    ]
    labels = {
        row.sample_id: row
        for row in (
            AggregatedLabel.model_validate_json(line)
            for line in (run / "aggregated_labels.jsonl").read_text().splitlines()
        )
    }
